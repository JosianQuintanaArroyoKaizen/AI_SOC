...
if [ "$USE_EXISTING" = false ]; then
  ...
  # Find the current API endpoint pattern in the file
  CURRENT_ENDPOINT=$(grep -oP "https://[a-z0-9]+\.execute-api\.[a-z0-9-]+\.amazonaws\.com" dashboard/static/index.html | head -n 1 || echo "")
  
  if [ -z "$CURRENT_ENDPOINT" ]; then
    echo "❌ Error: Could not find API endpoint pattern in index.html"
    echo "   Expected format: https://*.execute-api.*.amazonaws.com"
    echo "   Please check dashboard/static/index.html line ~527"
    exit 1
  fi
  
  echo "   Found current endpoint: $CURRENT_ENDPOINT"
  echo "   Replacing with: $API_ENDPOINT"
  
  # Replace the API endpoint
  sed "s|${CURRENT_ENDPOINT}|${API_ENDPOINT}|g" \
    dashboard/static/index.html > /tmp/index_deploy.html
  
  # Verify the replacement succeeded
  NEW_ENDPOINT=$(grep -oP "https://[a-z0-9]+\.execute-api\.[a-z0-9-]+\.amazonaws\.com" /tmp/index_deploy.html | head -n 1 || echo "")
  
  if [ "$NEW_ENDPOINT" != "$API_ENDPOINT" ]; then
    echo "❌ Error: API endpoint replacement failed"
    echo "   Expected: $API_ENDPOINT"
    echo "   Got: $NEW_ENDPOINT"
    rm -f /tmp/index_deploy.html
    exit 1
  fi
